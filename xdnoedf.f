...
c-----------------------------------------------------------------------
c
      subroutine xdnoedf (aom,nmo,ncent,naom,ngroup,nfugrp,ifugrp,pcut,
     &                   epsbond,lr,lw,lerr)
c
c.......................................................................
c
...
c
      if (.not.allocated(eignz)) then
        ...
        if (ier.ne.0) stop 'xdnoedf.f: Cannot allocate eignz()'
      endif
      nzeig=izero
      do i=1,nmo
...
c
      if (.not.allocated(sgn)) then
        ...
        if (ier.ne.0) stop 'xdnoedf.f: Cannot allocate sgn()'
      endif
      do k=1,ngroup-1
        do i=1,nzeig
          i1=eignz(i)
...
c
      if (.not.allocated(resncb)) then
        ...
        if (ier.ne.0) stop 'xdnoedf.f: Cannot allocate resncb()'
      endif
c
...
c
      ...
      if (nprev.lt.npab) then
        write (stderr,*) 'xdnoedf.f: NPREV < NPAB returned by rnprobs.f'
        stop
      endif
c
      if (.not.allocated(resnca)) then
        allocate (resnca(npab,ngroup),stat=ier)
        if (ier.ne.0) stop 'xdnoedf.f: Cannot allocate resnca()'
      endif
...
*       if (ier.ne.0) stop 'xdnoedf.f: Cannot deallocate aom()'
*     endif
      if (allocated(sg)) then
        deallocate (sg,stat=ier)
        if (ier.ne.0) stop 'xdnoedf.f: Cannot deallocate sg()'
      endif
      if (allocated(sdiag)) then
        deallocate (sdiag,stat=ier)
        if (ier.ne.0) stop 'xdnoedf.f: Cannot deallocate sdiag()'
      endif
      if (allocated(bdiag)) then
        deallocate (bdiag,stat=ier)
        if (ier.ne.0) stop 'xdnoedf.f: Cannot deallocate bdiag()'
      endif
      if (allocated(seigen)) then
        deallocate (seigen,stat=ier)
        if (ier.ne.0) stop 'xdnoedf.f: Cannot deallocate seigen()'
      endif
      if (allocated(wdiag)) then
        deallocate (wdiag,stat=ier)
        if (ier.ne.0) stop 'xdnoedf.f: Cannot deallocate wdiag()'
      endif
      if (allocated(eignz)) then
        deallocate (eignz,stat=ier)
        if (ier.ne.0) stop 'xdnoedf.f: Cannot deallocate eignz()'
      endif
      if (allocated(sgn)) then
        deallocate (sgn,stat=ier)
        if (ier.ne.0) stop 'xdnoedf.f: Cannot deallocate sgn()'
      endif
      if (allocated(eleca)) then
        deallocate (eleca,stat=ier)
        if (ier.ne.0) stop 'xdnoedf.f: Cannot deallocate eleca()'
      endif
      if (allocated(resncb)) then
        deallocate (resncb,stat=ier)
        if (ier.ne.0) stop 'xdnoedf.f: Cannot deallocate resncb()'
      endif
      if (allocated(resnca)) then
        deallocate (resnca,stat=ier)
        if (ier.ne.0) stop 'xdnoedf.f: Cannot deallocate resnca()'
      endif
      if (allocated(mocore)) then
        deallocate (mocore,stat=ier)
        if (ier.ne.0) stop 'xdnoedf.f: Cannot deallocate mocore()'
      endif
c
      call timer (4,idno,'_xdnoedf  ',-1)
      return
c
