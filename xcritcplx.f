...
c-----------------------------------------------------------------------
c
      subroutine xcritcplx (pcut,npais,npbis,ngroup,nmoa,nmob,lw,
...
c
      real(kind=8), allocatable,dimension (:,:) :: rpow,ipow
...
c
      call timer (2,ipid,'_xcritcplx',-1)
c
...
c
      ...
      if (.not.allocated(probr)) then
        ...
        if (ier.ne.0) stop 'xcritcplx.f: Cannot allocate probr()'
      endif
      if (.not.allocated(rsrs)) then
        allocate (rsrs(npmax,ngroup),stat=ier)
        if (ier.ne.0) stop 'xcritcplx.f: Cannot allocate rsrs()'
      endif
c
...
c
 ...
 22   continue
      ...
      if (.not.allocated(oveaa)) then
        ...
        if (ier.ne.0) stop 'xcritcplx.f: Cannot allocate oveaa()'
      endif
      if (.not.allocated(ww)) then
        allocate (ww(nmois),stat=ier)
        if (ier.ne.0) stop 'xcritcplx.f: Cannot allocate ww()'
      endif
      if (.not.allocated(indx)) then
        allocate (indx(nmois),stat=ier)
        if (ier.ne.0) stop 'xcritcplx.f: Cannot allocate indx()'
      endif
      if (.not.allocated(am)) then
        allocate (am(npab,npab),stat=ier)
        if (ier.ne.0) stop 'xcritcplx.f: Cannot allocate am()'
      endif
      if (.not.allocated(rpow)) then
        allocate (rpow(npab,ngroup),stat=ier)
        if (ier.ne.0) stop 'xcritcplx.f: Cannot allocate rpow()'
      endif
      if (.not.allocated(ipow)) then
        allocate (ipow(npab,ngroup),stat=ier)
        if (ier.ne.0) stop 'xcritcplx.f: Cannot allocate ipow()'
      endif
      if (.not.allocated(ipvt)) then
        allocate (ipvt(npab),stat=ier)
        if (ier.ne.0) stop 'xcritcplx.f: Cannot allocate ipvt()'
      endif
      if (.not.allocated(w)) then
        allocate (w(npab),stat=ier)
        if (ier.ne.0) stop 'xcritcplx.f: Cannot allocate w()'
      endif
      if (.not.allocated(probs)) then
        allocate (probs(npab),stat=ier)
        if (ier.ne.0) stop 'xcritcplx.f: Cannot allocate probs()'
      endif
c
...
c
      ...
      singular = .true.
      do while (singular)
        am(1:npab,1:npab)=cmplx(0.0d+00,0.0d+00)
        rpow=0.0d+00
        ipow=0.0d+00
        do i=1,npab
          do k=1,ngroup-1
            rpow(i,k)=2.0d+00*random(idum)-1.0d+00
            ipow(i,k)=2.0d+00*random(idum)-1.0d+00
          enddo
        enddo
        do i=1,npab
          do j=1,npab
            aco=cmplx(1.0d+00,0.0d+00)
            do k=1,ngroup-1
              aco=aco*cmplx(rpow(i,k),ipow(i,k))**rsrs(j,k)
            enddo
            am(i,j) = aco
          enddo
        enddo
      ...
      do n=1,npab
        do m=1,nmois
          do k=1,nmois
            ...
            do igr=1,ngroup-1
               dumaom=dumaom+cmplx(rpow(n,igr),ipow(n,igr))*sg(igr,m,k)
            enddo
            oveaa(m,k)=dumaom
          enddo
        enddo
...
c
      ...
      if (.not.allocated(resalp)) then
        ...
        if (ier.ne.0) stop 'xcritcplx.f: Cannot allocate resalp()'
      endif
      if (.not.allocated(igtzero)) then
        allocate (igtzero(npab,2),stat=ier)
        if (ier.ne.0) stop 'xcritcplx.f: Cannot allocate igtzero()'
      endif
      nnonz(ispin) = 0
      sumgt = 0.0d+00
     ...
     &         (resalp(igr,nnz,ispin),igr=1,ngroup)
      ...
      deallocate (rpow,am,ipvt,w,oveaa,ww,indx)
      if (ispin.eq.1.and.nspin.eq.2) then
        ispin = 2
        if (allocated(am)) then
          deallocate (am,stat=ier)
          if (ier.ne.0) stop 'xcritcplx.f: Cannot deallocate am()'
        endif
        if (allocated(rpow)) then
          deallocate (rpow,stat=ier)
          if (ier.ne.0) stop 'xcritcplx.f: Cannot deallocate rpow()'
        endif
        if (allocated(ipvt)) then
          deallocate (ipvt,stat=ier)
          if (ier.ne.0) stop 'xcritcplx.f: Cannot deallocate ipvt()'
        endif
        if (allocated(w)) then
          deallocate (w,stat=ier)
          if (ier.ne.0) stop 'xcritcplx.f: Cannot deallocate w()'
        endif
        if (allocated(oveaa)) then
          deallocate (oveaa,stat=ier)
          if (ier.ne.0) stop 'xcritcplx.f: Cannot deallocate oveaa()'
        endif
        if (allocated(ww)) then
          deallocate (ww,stat=ier)
          if (ier.ne.0) stop 'xcritcplx.f: Cannot deallocate ww()'
        endif
        if (allocated(indx)) then
          deallocate (indx,stat=ier)
          if (ier.ne.0) stop 'xcritcplx.f: Cannot deallocate indx()'
        endif
        if (allocated(probs)) then
          deallocate (probs,stat=ier)
          if (ier.ne.0) stop 'xcritcplx.f: Cannot deallocate probs()'
        endif
        goto 22
      else
        npbis = npais
...
c
      return
c
      call timer (4,ipid,'_xcritcplx',-1)
      return
c
c.....Formats.
c
