...
c-----------------------------------------------------------------------
c
      subroutine sdwlewis (aom,mal,mbe,nfrag,atoms_in_frag,
...
c-----------------------------------------------------------------------
c
      call timer (2,isdwlewis,'_sdwlewis ',-1)
c
...
c
     ...
     &            stop '# sdwlewis.f: Increase first dimension of iab'
...
c
      ...
      ocup(1:nloc)=2d0
      if (ier.ne.0) stop '# sdwlewis.f: Cannot allocate ocup()'
...
c
      ...
      if (ier.ne.0) stop '# sdwlewis.f: Cannot deallocate ocup()'
c
...
*     deallocate (onerdm   )
      ...
      if (ispin.eq.2) then
        ...
        enddo
        write (lw,987) sumeos
        call timer (4,isdwlewis,'_sdwlewis ',-1)
        return
      endif
      ispin = 2
      goto 1
...
c
 ...
 434  format (' #',/,' # sdwlewis.f: Unable to localize all the MOs',
 ...
 435  format (' #',
     & /,' # sdwlewis.f: !! WARNING: Too many localized MOs !',/,
     &   ' # ++++++++++++++++++++++++++',/,
...
*    &   ' Localized MOs with very close eigenvalues:',/,' # ',
 436  format (' #',
     ...
     & '-fragment MOs')
 100  format (' # sdwlewis.f: File ',a,' NOT FOUND')
